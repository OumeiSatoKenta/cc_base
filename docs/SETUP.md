...
- **AWS 系 MCP サーバー**: Claude Code が AWS API を呼び出してリソース情報を取得する（マネージド `aws-mcp` を `mcp-proxy-for-aws` 経由で使用。SigV4 署名に AWS 認証情報が必要）
...
`.mcp.json` の `aws-mcp` は `AWS_PROFILE=your-profile` を参照する（`mcp-proxy-for-aws` がこのプロファイルの認証情報で SigV4 署名する）。このプロファイルを設定する:
...
| **aws-mcp** | `uvx` (stdio, `mcp-proxy-for-aws`) | マネージド AWS MCP Server: API 呼び出し・ドキュメント検索（read-only）。旧 `aws-api-mcp-server` / `awsknowledge` を統合 |
...
- **uvx 系**（serena, aws-mcp（`mcp-proxy-for-aws`）, well-architected-security）: uv が必要（install-tools.sh で自動インストール済み）
- **codex**: OpenAI Codex CLI が必要（install-tools.sh で自動インストール済み）
- **AWS 系 MCP サーバー**: AWS 認証情報の設定が必要（セクション3を参照）。`aws-mcp` は初回接続時にプロジェクトスコープ MCP の承認が必要（`claude` 実行時に承認）
...
> **注**: 旧 `awsknowledge` MCP（アーキテクチャ・ベストプラクティス）はマネージド `aws-mcp` の `search_documentation` / `read_documentation` に統合された。ツール重複を避けるため、プラグイン側の `awsknowledge` は使用せず `aws-mcp` を利用すること。
