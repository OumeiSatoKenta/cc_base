...
| AWS サービスの設計・ベストプラクティス・ドキュメント検索 | マネージド `aws-mcp` の `search_documentation` / `read_documentation`（旧 `awsknowledge` を統合） |
| AWS 料金・IaC | `deploy-on-aws` プラグイン（`awspricing` / `awsiac` MCP） + `deploy` スキル |
| AWS 構成図の生成 | `deploy-on-aws` 同梱の `aws-architecture-diagram` スキル |
| AWS API 読み取り確認 | マネージド `aws-mcp` の `call_aws`（read-only） |
